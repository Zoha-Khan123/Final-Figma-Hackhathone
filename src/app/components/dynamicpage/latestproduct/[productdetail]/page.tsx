import { latestProduct } from "@/app/components/latestProduct"; // Adjust the path as necessary
import Image from "next/image";
import Link from "next/link";

export default function ProductDetail({params}: {params: { productdetail: string };}) {
  
    const selectedProduct = latestProduct.find((p) => p.id === params.productdetail);
    ...
    const relatedProducts = latestProduct.filter((p) => p.id !== selectedProduct.id);
      ...
      <p className="text-lg font-bold">Price: ${selectedProduct.price.toFixed(2)}</p>
        ...
        <div className="grid max-w-[1200px] sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 px-6 py-6 gap-5 mb-10">
          {relatedProducts.slice(0, 5).map((product) => (
            <Link key={product.id} href={`/components/dynamicpage/latestproduct/${product.id}`}>
              <div className="w-full h-40 relative bg-white ">
                <Image
                  ...
                  />
                <p>{product.title}</p>
                <span>${product.price.toFixed(2)}</span><del className="ml-3 px-2 text-red-500">$65</del>
              </div>
            </Link>
          ))}
          </div>
